...
export function catLabel(cat) {
  const map = { web: 'Web Dev', backend: 'Backend / API', database: 'Database', ml: 'Machine Learning', other: 'Other' };
  return map[cat] || cat;
}

export function spinner() { return '<div class="spinner" role="status" aria-label="Loading…"></div>'; }

export function errBanner(msg) {
  return `<div class="badge badge-red" style="margin:40px auto;display:flex;width:fit-content;"><i class="fas fa-exclamation-triangle"></i> ${escHtml(msg)}</div>`;
}
...
export function fmtDate(d) { if (!d) return 'Present'; return new Date(d).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }); }
