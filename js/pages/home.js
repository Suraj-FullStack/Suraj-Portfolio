...
export async function renderHome(el, summary) {
    ...
    <div class="container">
      <div class="hero-grid">
        <div>
          ...
          <h1 class="hero-name fade-in">Hi, I'm<br><span class="grad-text">Suraj Kumar</span><br>Shah</h1>
          <p class="hero-role fade-in"><span id="typed" class="typed"></span><span class="cursor">|</span></p>
          <p class="hero-bio fade-in">${escHtml(about?.bio || 'A passionate Computer System Engineering student at ISMT, Kathmandu, specialising in Python & Django backend development.')}</p>
        ...
        <div class="hero-avatar-wrap">
          <div class="avatar-orbit">
            <div class="avatar-ring"><div class="avatar-inner">${about?.profile_image ? `<img src="${about.profile_image}" alt="Suraj Kumar Shah">` : '👨‍💻'}</div></div>
  ...
  <section class="section section-alt">
    <div class="container">
      <div class="text-center"><span class="section-label"><i class="fas fa-certificate"></i> Credentials</span>
...
function buildChips(cats) { if (!cats?.length) return defaultChips(); return cats.flatMap(cat => cat.skills.map(s => `<span class="skill-chip">${s.icon ? `<i class="${escHtml(s.icon)}"></i>` : '<i class="fas fa-cog"></i>'}${escHtml(s.name)}</span>`)).join(''); }
function defaultChips() { const chips = [['fab fa-python text-warning','Python'],['fas fa-server text-success','Django'],['fas fa-code text-info','DRF'],['fas fa-database text-primary','PostgreSQL'],['fab fa-html5 text-danger','HTML5'],['fab fa-css3-alt text-info','CSS3'],['fab fa-js-square text-warning','JavaScript'],['fab fa-bootstrap','Bootstrap'],['fab fa-git-alt text-danger','Git'],['fab fa-github','GitHub']]; return chips.map(([ic,n]) => `<span class="skill-chip"><i class="${ic}"></i>${n}</span>`).join(''); }

function buildProjectCards(projects) { if (!projects?.length) return defaultProjects(); return projects.map(p => `
...
function defaultProjects() { return [ { emoji:'📸', cat:'Web Dev', title:'Boundless Moments Photography', desc:'Responsive photography portfolio with SEO & accessibility.', stack:'HTML5, CSS3, JavaScript' }, { emoji:'🏨', cat:'Database', title:'Hotel Room Reservation System', desc:'Desktop app for Hidden Paradise Hotel with PostgreSQL backend.', stack:'Python, Tkinter, PostgreSQL' }, { emoji:'🌐', cat:'Backend', title:'Portfolio REST API + Frontend', desc:'Django REST API + Vanilla JS SPA with separated architecture.', stack:'Django, DRF, JavaScript' } ].map(p => `
    <article class="project-card fade-in">
      <div class="project-thumb">${p.emoji}<span class="thumb-cat">${p.cat}</span><span class="thumb-featured">⭐ Featured</span></div>
      <div class="project-body"><h3 class="project-title">${p.title}</h3><p class="project-desc">${p.desc}</p><div class="tech-wrap">${p.stack.split(',').map(t => `<span class="tech-tag">${t.trim()}</span>`).join('')}</div><div class="project-links"><button class="btn btn-primary btn-sm" onclick="navigate('projects')"><i class="fas fa-eye"></i> Details</button></div></div>
    </article>`).join(''); }

function buildCertCards(certs) { if (!certs?.length) return defaultCerts(); return certs.map(c => `<div class="cert-card fade-in"><div class="cert-icon" style="background:${escHtml(c.badge_color)}22;color:${escHtml(c.badge_color)}"><i class="${escHtml(c.icon)}"></i></div><div><div class="cert-name">${escHtml(c.name)}</div><div class="cert-issuer">${escHtml(c.issuer)}</div><div class="cert-date">${c.date_issued || ''}</div></div></div>`).join(''); }
function defaultCerts() { const c = [ { icon:'fas fa-graduation-cap', color:'#10b981', name:'Full Stack Python Django', issuer:'Mind Risers Institute of Technology', date:'May 2026' }, { icon:'fas fa-shield-alt', color:'#ef4444', name:'Cyber Security Workshop', issuer:'KMC Students Committee', date:'Mar 2022' }, { icon:'fas fa-search', color:'#f59e0b', name:'5-Day SEO Challenge', issuer:'ORKA SOCIALS', date:'Jun 2024' }, { icon:'fas fa-child', color:'#06b6d4', name:'Child Rights Toolkit', issuer:'UNICEF', date:'2024' }, { icon:'fas fa-comments', color:'#6366f1', name:'Building Communication', issuer:'Kathmandu Model School', date:'2022' } ]; return c.map(cc => `<div class="cert-card fade-in"><div class="cert-icon" style="background:${cc.color}22;color:${cc.color}"><i class="${cc.icon}"></i></div><div><div class="cert-name">${cc.name}</div><div class="cert-issuer">${cc.issuer}</div><div class="cert-date">${cc.date}</div></div></div>`).join(''); }
/**
 * home.js – Renders the landing page using API summary data.
 */
import { escHtml, techTag, catEmoji, catLabel, typewriter } from '../utils.js';
        ...
        <div class="hero-avatar-wrap">
          <div class="avatar-orbit">
            ...
            <div class="orbit-pill top-r">
              <i class="fab fa-python" style="color:#f59e0b"></i> Django Dev
            </div>
            <div class="orbit-pill bot-l">
              <i class="fas fa-database" style="color:#06b6d4"></i> PostgreSQL
            </div>
          </div>
        </div>
